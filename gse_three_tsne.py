...
# LOCAL CONFIG
# =========================
INPUT_PATH = "thursday_with_embeddings.json"   # <-- IMPORTANT: use embedded file
OUTPUT_FRONTEND = "thursday_tsne.json"
OUTPUT_BACKEND = ""
...
# Labels for Data
# =========================
Code = [d.get("Code", "").strip() for d in entries]
Data = [d.get("Data", "").strip() for d in entries]

labels = [
    f"{n}: {c[:120]}..." if len(c) > 120 else f"{n}: {c}"
    for n, c in zip(Code, Data)
]

X = np.array([d["embedding"] for d in entries], dtype=np.float32)

n, dim = X.shape
print(f"Loaded {n} Codes with embeddings (dim={dim}).")
